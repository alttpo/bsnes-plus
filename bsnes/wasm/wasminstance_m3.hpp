...
public:
  bool func_find(const std::string &i_name, std::shared_ptr<WASMFunction> &o_func) override;
  bool func_invoke(const std::shared_ptr<WASMFunction>& fn, uint32_t i_retc, uint32_t i_argc, uint64_t *io_stack) override;
